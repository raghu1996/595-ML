...
submit_config.psnr = {"stddev":[],"avg_psnr":[]}
desc = "autoencoder"
...
datasets = {
    ...
    'bsd300': dnnlib.EasyDict(dataset_dir='datasets/bsd300_valid'),
    'set14':  dnnlib.EasyDict(dataset_dir='datasets/set14'),
    'dynamic_scenes': dnnlib.EasyDict(dataset_dir='/content/drive/MyDrive/ML-Project-new/noise2noise/datasets/Dynamic_scenes/Validate/Street/')
}

default_validation_config = datasets['bsd300']
#default_validation_config = datasets['dynamic_scenes']
...
if __name__ == "__main__":
    ...
    def validate(args):
        ...
        ##Added by Raghu
        if args.noise_stddev is None:
            if args.noise=="gaussian":
                gaussian_noise_config.validation_stddev = 25.0
                print("Validating with gaussian_noise_stddev: ",gaussian_noise_config.validation_stddev)
            elif args.noise=="poisson":
                poisson_noise_config.lam_max = 50.0
                print("Validating with poisson_noise_lammax: ",gaussian_noise_config.lam_max)
            validate_config.network_snapshot = args.network_snapshot
            dnnlib.submission.submit.submit_run(submit_config, **validate_config)
        else:
            if args.noise=="gaussian":
                for stddev in args.noise_stddev.split(","):
                    gaussian_noise_config.validation_stddev = float(stddev)
                    submit_config.psnr["stddev"].append(float(stddev))
                    print("Validating with gaussian_noise_stddev: ",gaussian_noise_config.validation_stddev)
                    validate_config.network_snapshot = args.network_snapshot
                    dnnlib.submission.submit.submit_run(submit_config, **validate_config)
            elif args.noise=="poisson":
                for lam_max in args.noise_stddev.split(","):
                    poisson_noise_config.lam_max = float(lam_max)
                    submit_config.psnr["stddev"].append(float(lam_max))
                    print("Validating with poisson_noise_stddev: ",poisson_noise_config.lam_max)
                    validate_config.network_snapshot = args.network_snapshot
                    dnnlib.submission.submit.submit_run(submit_config, **validate_config)
    ...
    def validate_video(args):
        ...
        if args.dataset_dir is None:
            ...
            isExist = os.path.exists(dataset_dir)
            if not isExist:
                # Create a new directory because it does not exist 
                os.makedirs(dataset_dir)
                print("Dumping frames to ",dataset_dir)
        ##Code to convert videos to frames and save it in dataset_dir/frames dir
        videos = glob.glob(args.video_dir)
        videos_split = args.video_dir.split("/")
        videos_gt = "/".join(videos_split[:-3])
        videos_gt = videos_gt + "/camera_stationary/"+videos_split[-2] 
        if len(videos) == 0:
            print ('No videos found in path args.videos')
            sys.exit(1)
        else:
            video_count = -1
            for video in videos:
                ...
                video_gt = videos_gt+"/"+video.split("/")[-1].replace("_moving_","_static_",1)
                vidcap_gt = cv2.VideoCapture(video_gt)
                sec = 0
                frameRate = 0.5 #//it will capture image in each 0.5 second
                count=1        
                vidcap.set(cv2.CAP_PROP_POS_MSEC,sec*1000)
                vidcap_gt.set(cv2.CAP_PROP_POS_MSEC,sec*1000)
                hasFrames,image = vidcap.read()
                hasFrames_gt,image_gt = vidcap_gt.read()
                if hasFrames:
                    #print("Dumping frames from ",video)
                    cv2.imwrite(dataset_dir+str(video_count)+"_"+str(count)+".jpg", image)     # save frame as JPG file
                    print("Noisy image shape ",image.shape[2],image.shape[1],image.shape[0])
                while hasFrames:
                    count = count + 1
                    sec = sec + frameRate
                    sec = round(sec, 2)
                    ...
                    hasFrames,image = vidcap.read()
                    if hasFrames:
                        cv2.imwrite(dataset_dir+str(video_count)+"_"+str(count)+".jpg", image) 
                count = 1
                sec = 0
                if hasFrames_gt:
                    cv2.imwrite(dataset_dir+"gt_"+str(video_count)+"_"+str(count)+".jpg", image_gt) # save frame as JPG file
                    print("GT image shape ",image_gt.shape[2],image_gt.shape[1],image_gt.shape[0])
                while hasFrames_gt:
                    count = count+1
                    sec = sec+frameRate
                    sec = round(sec,2)
                    ...
                    hasFrames_gt,image_gt = vidcap_gt.read()
                    if hasFrames_gt:
                        cv2.imwrite(dataset_dir+"gt_"+str(video_count)+"_"+str(count)+".jpg", image_gt)
    ...
    parser_validate.add_argument('--noise_stddev',nargs='?',default=0.0,help='Std dev for gaussian noise/lamd max for poission noise')
    ...
    parser_validate_video = subparsers.add_parser('validate_video', help='Run a set of images through the network')
    parser_validate_video.add_argument('--dataset-dir', help='Load all images from a directory (*.png, *.jpg/jpeg, *.bmp)')
    parser_validate_video.add_argument('--video-dir', help='Load all videos from a directory (*.mp4)')
    parser_validate_video.add_argument('--network-snapshot', help='Trained network pickle')
    parser_validate_video.add_argument('--noise', default='gaussian', help='Type of noise corruption (one of: gaussian, poisson)')
    parser_validate_video.add_argument('--noise_stddev',nargs='?',default=0.0,help='Std dev for gaussian noise/lamd max for poission noise')
    parser_validate_video.set_defaults(func=validate_video)
    ...
    for values in submit_config.psnr["avg_psnr"]:
        ...
        count = count+1
    plt.plot(submit_config.psnr["stddev"],submit_config.psnr["avg_psnr"])
    if args.noise=="gaussian":
        plt.title("Gaussian Noise: PSNR vs noise sigma")
        plt.xlabel("Noise sigma")
        if len(submit_config.psnr["avg_psnr"]) !=0:
            plt.yticks(np.arange(min(submit_config.psnr["avg_psnr"]),max(submit_config.psnr["avg_psnr"]),2))
    elif args.noise=="poisson":
        plt.title("Poisson Noise: PSNR vs lam_max")
        plt.xlabel("lam_max")
        if len(submit_config.psnr["avg_psnr"]) !=0:
            plt.yticks(np.arange(min(submit_config.psnr["avg_psnr"]),max(submit_config.psnr["avg_psnr"]),2))
    plt.ylabel("PSNR")
