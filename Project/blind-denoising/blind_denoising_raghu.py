...
def blind_denoising(**args):
    ...
    if args['network']=="dncnn/logs/DnCNN-S-25/net.pth":
    ...
    if args['enable_flow']==1:
        criterion = WarpedLoss()   
    elif args['enable_flow']==0:
        criterion = Loss()       
    ...
    if args['input'][-4:] == 'tiff' or args['input'][-3:] == 'tif':
        ...
        imorig = cv2.imread(args['input'] % (args['first']),cv2.IMREAD_GRAYSCALE)
    ...
    for i in range(args['first']+1, args['last']+1):
        ...
        if args['input'][-4:] == 'tiff' or args['input'][-3:] == 'tif':
            ...
            curr_frame = cv2.imread(args['input'] % (i),cv2.IMREAD_GRAYSCALE)
        ...
        for it in range(args['iter']):
            ...
            if args['enable_flow']==1:
                ...
                loss = criterion(out_train,prev_frame_var)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if args['ref'][-4:] == 'tiff' or args['ref'][-3:] == 'tif':
            ...
            ref_frame = cv2.imread(args['ref'] % (i),cv2.IMREAD_GRAYSCALE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable_flow", type=int, default=1,
                        help='Enable/disable optical flow')
